...
public class PopupMenuCustomLayout {
    ...
    public void show(View anchorView/*, int gravity, int offsetX, int offsetY*/) {
        popupWindow.showAsDropDown( anchorView, 0, -1 * (anchorView.getHeight()));
    }
    ...
    public View getMenuItem(int index) {
        if (index >= 0 && index < ((LinearLayout) popupView).getChildCount()) {
            return ((LinearLayout) popupView).getChildAt(index);
        }
        return null;
    }
}